  ...
  <div class="d-flex flex-column px-20">
    ...
    <form @submit.prevent="addProduct" class="flex flex-col gap-6 mt-4">
      ...
      <div class="flex flex-col gap-2">
        ...
        <input type="file" name="image" accept="image/*" class="text-gray-400" @change="getFileName" ref="imageInput" />
      </div>
      <button type="submit" class="bg-black py-2 w-1/4 text-white self-center rounded-md mt-8">Add Product</button>
    </form>
  </div>
</template>
...
const uploadFile = async () => {
  ...
  const { data, error } = await supabase.storage.from('product-images').upload(fileName.value, file);
  if (error) {
    console.log(error);
  }
};
